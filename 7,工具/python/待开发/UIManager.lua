...
local fileDir = "../Client/src/__ClickButton.lua"
local file = io.open(fileDir, "w")
if file then
	file:close()
end
--统计截至时间,在此时候的数据还未被输出
local statisticsEndTime = 0;
--统计数据表
local statisticsDatas = {}

local function statisticsMessage(messageName,messageType)
	local nowTime = tonumber(os.date("%H%M%S", kod.util.Time.now()))

	--处理数据消息
	local tempBool = false
	for i,v in ipairs(statisticsDatas) do
		if v.message == messageName then
			--这个消息已存在,叠加次数
			v.counts = v.counts + 1
			tempBool = true
			break
	...
	if not tempBool then
		table.insert(statisticsDatas,{time = nowTime,counts = 1,message = messageName,messageStr = os.date("%H:%M:%S", kod.util.Time.now())..messageType..messageName})
	end
	
	local tempTime = kod.util.Time.now() * 1000
	if statisticsEndTime == 0 or (tempTime - statisticsEndTime > 1000) then
		--输出未输出的统计数据
		local tempEndTime = tonumber(os.date("%H%M%S", kod.util.Time.now()))
		local file = io.open(fileDir, "a+")
		for i,v in ipairs(statisticsDatas) do
			if statisticsEndTime < v.time then
				file:write(v.messageStr..",共"..v.counts.."次 \n")
			end
		end
		statisticsEndTime = tempTime
		file:close()
	end
end
...
cc.exports.bindEventCallBack = function(register,callback,event)
	...
	local registerCallBack = function(sender, eventType)
		...
		if event == eventType then
			statisticsMessage(sender:getName()," 点击按钮: ");
			callback(sender);
		end
	end
