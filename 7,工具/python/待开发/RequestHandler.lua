...
local fileDir = "../Client/src/__Message.lua"
...
local function statisticsMessage(messageName,messageType)
	...
	for i,v in ipairs(statisticsDatas) do
		if v.message == messageName and nowTime == v.time then
			--这个消息已存在,叠加次数
			v.counts = v.counts + 1
			tempBool = true
			break
...
local function printMessage()
	...
	local tempStr = ""
	for i,v in ipairs(statisticsDatas) do
		tempStr = tempStr..v.messageStr..",共"..v.counts.."次 \n"
	end
	file:write(tempStr.."\n\n")
	print(tempStr)
	print("statisticsDatas",#statisticsDatas)
	file:close()
end
----------------------------
-- 客户端协议处理层
...
function RequestHandler:ctor(timeoutCallback)
	...
	cc.Director:getInstance():getScheduler():scheduleScriptFunc(printMessage,1,false)
end
...
function RequestHandler:onRequest(req)
	...
	statisticsMessage(req._protocol.class.__cname," 发送消息: ")
...
function RequestHandler:onResponse(resp)
	...
	statisticsMessage(resp._protocol.class.__cname," 发送消息: ")
